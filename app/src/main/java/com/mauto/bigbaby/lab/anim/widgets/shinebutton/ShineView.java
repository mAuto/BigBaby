...
public class ShineView extends View {
    ...
    int tinyShineColor = colorRandom[2];
    ...
    public ShineView(Context context, final ShineButton shineButton, ShineParams shineParams) {
        ...
        paintTiny = new Paint();
        paintTiny.setColor(tinyShineColor);
        paintTiny.setStrokeWidth(5);
        paintTiny.setStyle(Paint.Style.STROKE);
        paintTiny.setStrokeCap(Paint.Cap.ROUND);
...
//                Log.e(">>>>>>", "ShineView --> " + "onAnimationUpdate"+" "+value);

                int tmpWidth = (int) (btnWidth * (value / 1.5f));
                int tmpHeight = (int) (btnHeight * (value / 1.5f));
                ...
                if (shineSize != 0 && shineSize > 0) {
                    ...
                    paint.setStrokeWidth((tmpWidth / 2) * (shineDistanceMultiple - value));
                    paintSmall.setStrokeWidth((tmpWidth / 3) * (shineDistanceMultiple - value));
                    paintTiny.setStrokeWidth((tmpWidth / 4) * (shineDistanceMultiple - value));
                }


                rectF.set(centerAnimX - (tmpWidth / (3 - shineDistanceMultiple) * value), centerAnimY - (tmpHeight / (3 - shineDistanceMultiple) * value), centerAnimX + (tmpWidth / (3 - shineDistanceMultiple) * value), centerAnimY + (tmpHeight / (3 - shineDistanceMultiple) * value));
                rectFSmall.set(centerAnimX - (tmpWidth / ((3 - shineDistanceMultiple) + distanceOffset) * value), centerAnimY - (tmpHeight / ((3 - shineDistanceMultiple) + distanceOffset) * value), centerAnimX + (tmpWidth / ((3 - shineDistanceMultiple) + distanceOffset) * value), centerAnimY + (tmpHeight / ((3 - shineDistanceMultiple) + distanceOffset) * value));
                rectFThird.set(centerAnimX - (tmpWidth / ((3 - shineDistanceMultiple) + distanceOffset * 2) * value), centerAnimY - (tmpHeight / ((3 - shineDistanceMultiple) + distanceOffset * 2) * value), centerAnimX + (tmpWidth / ((3 - shineDistanceMultiple) + distanceOffset * 2) * value), centerAnimY + (tmpHeight / ((3 - shineDistanceMultiple) + distanceOffset * 2) * value));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < shineCount; i++) {
            ...
            canvas.drawArc(rectFThird, 360f / shineCount * i + 1 - smallOffsetAngle * 2 + ((value - 1) * turnAngle), 0.1f, false, getConfigPaint(paintTiny));
...
//        canvas.drawPoint(centerAnimX, centerAnimY, paint);
    ...
    private void initShineParams(ShineParams shineParams, ShineButton shineButton) {
        ...
        tinyShineColor = shineParams.tinyShineColor;
